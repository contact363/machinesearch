"""
Corel Machines public API — full machine data for external use.
...
def _full_dict(m: Machine) -> dict:
    """Return every available field for a Corel Machines machine."""
    specs = m.specs or {}
...
@router.get("/machines")
async def list_corelmachine_machines(
    ...
    """List all Corel Machines listings with optional filters and pagination."""
...
async def get_corelmachine_machine(machine_id: str, db: AsyncSession = Depends(get_db)):
    """Get a single Corel Machines listing by UUID — all fields."""
    try:
    ...
    if not machine:
        raise HTTPException(status_code=404, detail="Corel Machines listing not found")

    return _full_dict(machine)


@router.delete("/machines/{machine_id}")
async def delete_corelmachine_machine(machine_id: str, db: AsyncSession = Depends(get_db)):
    """Delete a Corel Machines listing by UUID."""
    try:
        uid = _uuid.UUID(machine_id)
    except ValueError:
        raise HTTPException(status_code=400, detail="Invalid machine ID format")
...
@router.get("/filters")
async def corelmachine_filters(db: AsyncSession = Depends(get_db)):
