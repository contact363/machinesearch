...
async def main():
    ...
    # Any URL in our DB that is no longer on the EMUK listing page
    # gets its status checked. If 404 / no machine content → delete from DB.
    logger.info("Starting sync: checking for sold/removed EMUK machines...")
    ...
    async with SessionLocal() as session:
        result = await session.execute(
            sa_select(Machine.id, Machine.source_url).where(Machine.site_name == "emuk")
        )
        ...
        async with SessionLocal() as session:
            for machine_id, url in missing:
                try:
                    ...
                    has_label = soup_check.find("div", class_="machine-detail-label")
                    has_headline = soup_check.find("div", class_="machine-detail-headline")

                    if not has_label or not has_headline:
                        # No machine specs/title = sold / page emptied
                        await session.execute(sa_delete(Machine).where(Machine.id == machine_id))
                        await session.commit()
                        deleted_count += 1
    ...
    logger.info("Sync complete — deleted %d sold/removed EMUK machines from DB", deleted_count)
    logger.info("=" * 60)
